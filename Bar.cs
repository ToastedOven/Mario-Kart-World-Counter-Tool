...
public partial class Bar : ColorRect
{
    ...
    //     if (checkOffered)
    //     {
    //         int myValue = ButtonThing.buttons[spot].offeredValue + ButtonThing.buttons[spot + ControlManager.TRACKCOUNT].offeredValue;
    //         CustomMinimumSize = new Vector2(35,620f * (myValue / (float)ButtonThing.highestValue));
    //         countLabel.Text = myValue.ToString();   
    //     }
    //     else
    //     {
    //         int myValue = ButtonThing.buttons[spot].pickedValue + ButtonThing.buttons[spot + ControlManager.TRACKCOUNT].pickedValue;
    ...
    //     if (checkOffered)
    //     {
    //         var sortedBarOrder = bars.OrderByDescending(item => ButtonThing.buttons[item.spot].offeredValue + ButtonThing.buttons[item.spot + ControlManager.TRACKCOUNT].offeredValue).ToArray();
    ...
    //     else
    //     {
    //         var sortedBarOrder = bars.OrderByDescending(item => ButtonThing.buttons[item.spot].pickedValue + ButtonThing.buttons[item.spot + ControlManager.TRACKCOUNT].pickedValue).ToArray();
    //         sortedBarOrder = sortedBarOrder.Reverse().ToArray();
    //         foreach (var bar in sortedBarOrder)
    //         {
    //             bar.GetParent().MoveChild(bar, 0);
